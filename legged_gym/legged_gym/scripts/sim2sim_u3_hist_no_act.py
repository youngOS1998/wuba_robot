...
def runmj(policy):
    ...
    hist_obs = deque()
    for _ in range(5):
        hist_obs.append(np.zeros([1, 45-12], dtype=np.float32))
    ...
    kpP = 1.0 * np.array([300] * 12, dtype=np.float32)
    ...
    for _ in range(int(60 / 0.001)):
        ...
        if count_lowlevel % 20 == 0:
            ...
            action, latent, vel = action.detach().numpy(), latent.detach().numpy(), vel.detach().numpy()
            
            action_list.append(deepcopy(action))
            latent_list.append(deepcopy(latent))
            vae_vel_list.append(deepcopy(vel))
            lin_vel_list.append(deepcopy(v))
            # if wake_flag:
            #     print(v)
        ...
        # 1000hz -> 100hz
        # if count_lowlevel % 20 == 0:
        #     jtor_list.append(tau)
        #     taucmd_list.append(tau)
        #     # tau = np.clip(tau, -160, 160)  # Clamp torques
        #     # wo_knee_tau = tau[[0,1, 3,4, 6,7, 9,10]]
        #     # tau[[0,1, 3,4, 6,7, 9,10]] = np.clip(wo_knee_tau, -100, 100)  # Clamp torques
        #     # ctrl_full = np.concatenate( (tau, np.zeros([12], dtype=np.float32)) )
        #     jq_list.append(q)
        #     jdq_list.append(dq)
        #     target_q_list.append(target_q)
        #     if len(jq_list) >= 200 and not wake_flag:
        #         sim_ts_list.clear()
        #         jq_list.clear()
        #         jdq_list.clear()
        #         jtor_list.clear()
...
if __name__ == "__main__":
    ...
    policy = load_policy('/home/rl/Project/legged_gym_cjw/quadrl/legged_gym_58/logs/rough_x20/exported/policies')
