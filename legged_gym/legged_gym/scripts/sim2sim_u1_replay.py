...
def load_policy(logdir):
    actor = torch.jit.load(logdir + '/base_actor.pt')
    encoder = torch.jit.load(logdir + '/waq_encoder.pt')
    fc_mu = torch.jit.load(logdir + '/waq_encoder_mu.pt')
    fc_var = torch.jit.load(logdir + '/waq_encoder_var.pt')
    fc_vel = torch.jit.load(logdir + '/waq_encoder_vel.pt')
    ...
    while not glfw.window_should_close(self.window):
      ...
      while (self.data.time - simstart <= 1.0/60.0 and not self.pause_flag):
        ...
        obs_raw = {'q': q, 'dq': dq, 'quat': quat, 'v': v, 'omega': omega, 'gvec': gvec, 'is_registered': is_registered, 'calibrated': calibrated,
                   'lin_vel': lin_vel, 'ang_vel': omega}  #! 'ang_vel': omega???
...
def rlcontroller(policy, cmd_queue, shared_state):
    ...
    data_path = "/home/lenovo/Project/DreamWAQ_yiming_changes/legged_gym_58/legged_gym/scripts/Jun04_10-28-38/u1_sim_data_rl_1.npz"
    replay_data = load_data(data_path)
    action_replay = replay_data['action']
    latent_replay = replay_data['latent']
    action_count = 0
        ...
        if running:
            if cmd['mode'] == 1: # Calibration mode
                ...
                if calibrated:
                    ...
                    if action_count >= action_replay.shape[0]:
                        break
                    action, latent = action_replay[action_count], latent_replay[action_count]
                    action_count += 1
                    action[:], latent = action, latent
                    # print(action)
                    # breakpoint()
    ...
    sim_filename = 'u1_sim_replay_data'
    # sim_save_index = get_next_file_index(sim_filename)
    if mode_1_flag:
    ...
    np.savez(sim_filename, obs=np.array(obs_list), hist_obs=np.array(hist_obs_list), 
                action=np.array(action_list), latent=np.array(latent_list))
    policy_ts_list.clear()
    obs_list.clear()
    # hist_obs_list.clear()
    action_list.clear()
