...
def rlcontroller(policy, cmd_queue, shared_state):
    ...
    data_path = "/home/lenovo/Project/DreamWAQ_yiming_changes/legged_gym_58/legged_gym/scripts/play_data_rl.npz"
    replay_data = load_data(data_path)
    obs_replay = replay_data['obs']
    hist_obs_replay = replay_data['hist_obs']
        ...
        if running:
            if cmd['mode'] == 1: # Calibration mode
                ...
                if calibrated:
                    ...
                    if action_count >= obs_replay.shape[0]:
                        break
                    _action, _latent, _vel = policy(torch.tensor(hist_obs_replay[action_count], dtype=torch.float32), 
                                                    torch.tensor(obs_replay[action_count], dtype=torch.float32), 
                                                    lin_vel=None)
                    action[:], latent, vel = _action.detach().numpy(), _latent.detach().numpy(), _vel.detach().numpy()
                    print(np.abs(action_replay[action_count] - action).sum())
